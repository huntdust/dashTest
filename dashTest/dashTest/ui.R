library(shiny)
library(shinythemes)
library(plotly)
library(markdown)
library(rmarkdown)
library(knitr)

datapath <- "C:/Users/huntdust/Desktop/ShinyServer/"
analysispath <- "C:/home/dashTest/Analysis/"
datafiles <- list.files(datapath)
analysisfiles <- list.files(analysispath)

ui <- 
  navbarPage("Demo", collapsible = TRUE, inverse = TRUE, theme = shinytheme("spacelab"),
             tabPanel("SamplePlot",
                      fluidPage(
                        tabsetPanel(
                          tabPanel("Plot1",br(),
                                   sidebarLayout(
                                     sidebarPanel(
                                       radioButtons(
                                         inputId = "plotSelection",
                                         label = "Choose a plot",
                                         choices = c("Temperature",
                                                     "Humidity")), br()
                                     ),
                                     
                                     mainPanel(plotlyOutput(outputId = "samplePlot",height="600px"))
                                   )),
                          tabPanel("Plot2",br(),
                                   sidebarLayout(
                                     sidebarPanel(
                                       radioButtons(
                                         inputId = "plotSelection",
                                         label = "Choose a plot",
                                         choices = c("Temperature",
                                                     "Humidity")), br()
                                     ),
                                     
                                     mainPanel(imageOutput(outputId = "retPlot",height="600px"))
                                   ))
                        )
                      )),
             tabPanel("Temperature/Humidity Plots",
                      fluidPage(
                        tabsetPanel(
                          tabPanel("Temperature", br(),
                                   sidebarLayout(
                                     sidebarPanel(
                                       radioButtons(
                                         inputId = "timescaleTemp",
                                         label = "Timescale",
                                         choices = c("1 day",
                                                     "5 days",
                                                     "1 month", 
                                                     "all")), br(),
                                       
                                       radioButtons(
                                         inputId = "tempSensors",
                                         label = "Sensor",
                                         choices = c("Ambient_SD",
                                                     "Gryffindor",
                                                     "Reliability",
                                                     "Ambient_HP",
                                                     "Leakage_HP",
                                                     "Leakage2_HP",
                                                     "All"))
                                     ),
                                     mainPanel(
                                       plotlyOutput(outputId = "tempPlot", height = "1000px", width = "900px")
                                     )
                                   )),
                          tabPanel("Humidity", br(),
                                   sidebarLayout(
                                     sidebarPanel(
                                       radioButtons(
                                         inputId = "timescaleHum",
                                         label = "Timescale",
                                         choices = c("1 day",
                                                     "5 days",
                                                     "1 month", 
                                                     "all")), br(),
                                       
                                       radioButtons(
                                         inputId = "humSensors",
                                         label = "Sensor",
                                         choices = c("Ambient_SD",
                                                     "Gryffindor",
                                                     "Reliability",
                                                     "Ambient_HP",
                                                     "Leakage_HP",
                                                     "Leakage2_HP",
                                                     "All"))
                                       ),
                                     mainPanel(
                                       plotlyOutput(outputId = "humgraph", height = "800px", width = "900px")
                                       #imageOutput(outputId = "humPlot", height = "900px", width="900px")
                                       )
                                   ))
                        
                           
                        ))),
             tabPanel("HTML Demos",
                      fluidPage(
                        tabsetPanel(
                          tabPanel("HTML Sample", br(),
                                   sidebarLayout(
                                     sidebarPanel(
                                       selectInput(inputId = "analysisSelection",label= "Select Analysis File", choices = analysisfiles),
                                       br()),
                                     mainPanel(
                                       #includeMarkdown("\\\\pilly\\jaguar\\interface\\project\\Gryffindor\\Analysis Files - July 2021\\RF_Analysis_Part2.Rmd")
                                       #concat analysispath and selected input file
                                       #(paste(analysispath, anal))
                                     )
                                   )),
                          tabPanel("Knit", br(),
                                   sidebarLayout(
                                     sidebarPanel(
                                       #would use input$dataSelection to reference the selected file 
                                       selectInput(inputId = "dataSelection",label= "Select dataset", choices = datafiles),
                                       #reference "runAnalysis" button status with input$runAnalysis
                                       actionButton("runAnalysis", "Run Analysis",class = "btn-success"), br()),
                                     #render(input = "Gryffindor_Force_and_Position_Plots_MASTER.Rmd", "html_document", output_file = paste(input$dataSelection,".HTML")),
                                     #Analysis file output should go in main panel 
                                     #file to run analysis on : paste(datapath, datafiles[2])
                                     mainPanel(
                                       #define selectedPath 
                                       
                                       #render(input = "C:/home/dashTest/Gryffindor_Force_and_Position_Plots_MASTER.Rmd", "html_document", output_file = "test.html"),
                                       includeHTML('C:\\home\\dashTest\\test.html')
                                       
                                       
                                     )
                                   ))
                        ))), 
             tabPanel("Outreach"),
             tabPanel("About")
  )